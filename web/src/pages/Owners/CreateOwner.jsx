...
const CreateOwner = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await vehicleOwnerAPI.create(formData);
            navigate('/owners');
        } catch (err) {
            console.error('Error creating owner:', err);
            setErrors({ submit: t('owners.errors.create_failed') });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-primary">{t('owners.create_title')}</h1>
                    <p className="text-secondary mt-2">{t('owners.create_subtitle')}</p>
                </div>
            ...
            <Card>
                <CardHeader>
                    <h2 className="text-xl font-semibold text-primary">{t('owners.form_title')}</h2>
                </CardHeader>
                <CardBody>

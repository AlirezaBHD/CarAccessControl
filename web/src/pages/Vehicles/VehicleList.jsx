...
const VehicleList = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            setError(t('vehicles.errors.load_failed'));
    ...
    const columns = [
        ...
        {
            title: t('vehicles.fields.plate_number'),
            dataIndex: 'plateNumber',
            key: 'plateNumber',
            render: (plate) => (
                <PersianPlate plate={plate} />
            ),
        },
        {
            title: t('vehicles.fields.owner'),
            dataIndex: 'ownerFullName',
            key: 'ownerFullName',
            render: (fullName) => (
                <div>
                    <div className="font-medium text-secondary">{fullName}</div>
                </div>
            ),
        }
    ];
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-primary">{t('vehicles.title')}</h1>
                    <p className="text-secondary mt-2">{t('vehicles.subtitle')}</p>
                </div>
                <Button
                ...
                <Card hover>
                    <CardBody>
                        <div className="flex items-center">
                            ...
                            <div>
                                <p className="text-gray-600 text-sm">{t('vehicles.stats.total')}</p>
                ...
                <Card hover>
                    <CardBody>
                        <div className="flex items-center">
                            ...
                            <div>
                                <p className="text-gray-600 text-sm">{t('vehicles.stats.active')}</p>
                ...
                <Card hover>
                    <CardBody>
                        <div className="flex items-center">
                            ...
                            <div>
                                <p className="text-gray-600 text-sm">{t('vehicles.stats.active_owners')}</p>
                                <p className="text-2xl font-bold text-primary">
                                    {new Set(vehicles.map(vehicle => vehicle.ownerId)).size}
                                </p>
                            </div>
                        </div>
                    </CardBody>
                ...
                <Card hover>
                    <CardBody>
                        <div className="flex items-center">
                            ...
                            <div>
                                <p className="text-gray-600 text-sm">{t('vehicles.stats.avg_vehicles')}</p>
                                <p className="text-2xl font-bold text-primary">
                                    {(vehicles.length || 0).toFixed(1)}
                                </p>
                            </div>
                        </div>
                    </CardBody>
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <h2 className="text-xl font-semibold text-primary">{t('vehicles.list_title')}</h2>
                    </div>
                </CardHeader>
                <CardBody className="p-0 h-120 overflow-y-auto">
                    {vehicles.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-lg font-semibold text-secondary mb-2">{t('vehicles.no_vehicles')}</h3>
                            <p className="text-gray-600 mb-6">{t('vehicles.no_vehicles_desc')}</p>
